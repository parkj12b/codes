...
public:
	...
	unsigned int height(TaskItem* T) const;
	...
	TaskItem max(TaskItem* T) const;
	
	TaskItem min(TaskItem* T) const;
	
	bool exists(TaskItem* T, int key) const;
	
	bool exists(int key) const;
	
	void print(TaskItem* T) const;
};
#endif
